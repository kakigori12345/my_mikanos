...
EFI_STATUS ReadFile(EFI_FILE_PROTOCOL* file, VOID** buffer) {
  ...
  status = file->GetInfo(
    file, &gEfiFileInfoGuid, &file_info_size, file_info_buffer
  );
  if(EFI_ERROR(status)){
    return status;
  }
...
EFI_STATUS EFIAPI UefiMain(
  ...
  if(status == EFI_SUCCESS){
    ...
    UINTN volume_bytes = (UINTN)media->BlockSize * (media->LastBlock + 1); //ブロックデバイス全体のサイズ
  ...
  if(EFI_ERROR(status)){
    ...
    // ２回目の実行。map_key を取得しなおしたので今度は成功
    status = gBS->ExitBootServices(image_handle, memmap.map_key);
    if(EFI_ERROR(status)){
