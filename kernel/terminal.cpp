...
namespace {
  ...
  Error CleanPageMaps(LinearAddress4Level addr) {
    auto pml4_table = reinterpret_cast<PageMapEntry*>(GetCR3());
    auto pdb_table = pml4_table[addr.parts.pml4].Pointer();
    pml4_table[addr.parts.pml4].data = 0;
    if(auto err = CleanPageMap(pdb_table, 3)) {
      return err;
    }

    // PML4テーブルに1つしかエントリがない前提
    const auto pdb_addr = reinterpret_cast<uintptr_t>(pdb_table);
    const FrameID pdb_frame{pdb_addr / kBytesPerFrame};
    return memory_manager->Free(pdb_frame, 1);
  }
  ...
  while(true) {
    ...
    switch(msg->type) {
      case Message::kTimerTimeout:
        ...
        if(show_window && is_window_active) {
