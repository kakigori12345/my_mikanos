...
Error FrameBuffer::Initialize(const FrameBufferConfig& config){
  ...
  if(config_.frame_buffer){
    // 既に領域がある特殊なタイプ。流用するので内部バッファを0にしておく
    buffer_.resize(0);
  }
  else {
    buffer_.resize(
      // 1ピクセルに必要バイト数 × 高さ × 横 （単位：バイト）
...
Error FrameBuffer::Copy(Vector2D<int> pos, const FrameBuffer& src){
  ...
  uint8_t* dst_buf = 
    config_.frame_buffer + bytes_per_pixel * (config_.pixels_per_scan_line * copy_start_dst_y + copy_start_dst_x);
...
void FrameBuffer::Move(Vector2D<int> dst_pos, const Rectangle<int>& src){
  const auto bytes_per_pixel = BytesPerPixel(config_.pixel_format); //移動文サイズの1行分（バイト）
  const auto bytes_per_scan_line = BytesPerScanLine(config_);       //バッファサイズの1行分（バイト）
  ...
  // 下に移動
    ...
    // 移動先に書き換え前の領域があるとまずいので、下から上に書き換えていく
