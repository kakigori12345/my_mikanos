...
const int kBarSpeed = kCanvasWidth / 2 + 100; // pixels/sec
const int kBallSpeed = kBarSpeed - 80;
...
extern "C" void main(int argc, char** argv) {
  ...
  for (;;) {
    ...
    if (prev_timeout == 0) {
      const auto timeout = SyscallCreateTimer(TIMER_ONESHOT_REL, 1, 1000 / kFrameRate, "Blocks");
      prev_timeout = timeout.value;
    } else {
      prev_timeout += 1000 / kFrameRate;
      SyscallCreateTimer(TIMER_ONESHOT_ABS, 1, prev_timeout, "Blocks");
    }
